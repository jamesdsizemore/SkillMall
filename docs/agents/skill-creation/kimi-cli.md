...
| Install method | `cp -r skills/<category>/<slug> ~/.config/agents/skills/` |

## Frontmatter support

Kimi Code CLI follows the [AgentSkills open standard](https://agentskills.io/specification). Only standard fields are supported.
...
Kimi Code CLI has no native extensions beyond the standard. Skills created using the [universal format](universal.md) work without modification.

For SkillMall skills targeting Kimi Code CLI, use only:
...
Kimi Code CLI has no extensions beyond the AgentSkills standard. There is no agent-specific skill format.

If the user asks for a "Kimi Code CLI-specific" skill, create a universal skill and note in `compatibility` that it targets Kimi Code CLI:

```yaml
compatibility: "Intended for use with Kimi Code CLI."
```
...
Or describe the task — Kimi Code CLI loads the skill automatically when the description matches.

## Description budget behavior

Kimi Code CLI follows the universal best practice. All coding agents implement skill listing budgets. Keep `description` under 150 chars to avoid silent truncation when the context fills with many skills. Put the primary trigger phrase in the first 80 characters.

## Testing skills

1. Copy the skill to the appropriate path
2. Open Kimi Code CLI and type `/` to verify the skill appears
