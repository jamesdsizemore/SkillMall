...
| | Value |
|--|-------|
| Project path | `.rovodev/skills/` |
| Global path | `~/.rovodev/skills/` |
| Native extensions | No — follows AgentSkills standard only |
| Install method | `cp -r skills/<category>/<slug> ~/.rovodev/skills/` |

## Frontmatter support

Rovo Dev follows the [AgentSkills open standard](https://agentskills.io/specification). Only standard fields are supported.
...
Rovo Dev has no native extensions beyond the standard. Skills created using the [universal format](universal.md) work without modification.

For SkillMall skills targeting Rovo Dev, use only:
...
Rovo Dev has no extensions beyond the AgentSkills standard. There is no agent-specific skill format.

If the user asks for a "Rovo Dev-specific" skill, create a universal skill and note in `compatibility` that it targets Rovo Dev:

```yaml
compatibility: "Intended for use with Rovo Dev."
```
...
Or describe the task — Rovo Dev loads the skill automatically when the description matches.

## Description budget behavior

Rovo Dev follows the universal best practice. All coding agents implement skill listing budgets. Keep `description` under 150 chars to avoid silent truncation when the context fills with many skills. Put the primary trigger phrase in the first 80 characters.

## Testing skills

1. Copy the skill to the appropriate path
2. Open Rovo Dev and type `/` to verify the skill appears
