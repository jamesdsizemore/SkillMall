...
| Install method | `cp -r skills/<category>/<slug> ~/.codebuddy/skills/` |

## Frontmatter support

CodeBuddy follows the [AgentSkills open standard](https://agentskills.io/specification). Only standard fields are supported.
...
CodeBuddy has no native extensions beyond the standard. Skills created using the [universal format](universal.md) work without modification.

For SkillMall skills targeting CodeBuddy, use only:
...
CodeBuddy has no extensions beyond the AgentSkills standard. There is no agent-specific skill format.

If the user asks for a "CodeBuddy-specific" skill, create a universal skill and note in `compatibility` that it targets CodeBuddy:

```yaml
compatibility: "Intended for use with CodeBuddy."
```
...
Or describe the task — CodeBuddy loads the skill automatically when the description matches.

## Description budget behavior

CodeBuddy follows the universal best practice. All coding agents implement skill listing budgets. Keep `description` under 150 chars to avoid silent truncation when the context fills with many skills. Put the primary trigger phrase in the first 80 characters.

## Testing skills

1. Copy the skill to the appropriate path
2. Open CodeBuddy and type `/` to verify the skill appears
