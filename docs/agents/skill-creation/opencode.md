...
| Install method | `cp -r skills/<category>/<slug> ~/.config/opencode/skills/` |

## Frontmatter support

OpenCode follows the [AgentSkills open standard](https://agentskills.io/specification). Only standard fields are supported.
...
OpenCode has no native extensions beyond the standard. Skills created using the [universal format](universal.md) work without modification.

For SkillMall skills targeting OpenCode, use only:
...
OpenCode has no extensions beyond the AgentSkills standard. There is no agent-specific skill format.

If the user asks for a "OpenCode-specific" skill, create a universal skill and note in `compatibility` that it targets OpenCode:

```yaml
compatibility: "Intended for use with OpenCode."
```
...
Or describe the task — OpenCode loads the skill automatically when the description matches.

## Description budget behavior

OpenCode follows the universal best practice. All coding agents implement skill listing budgets. Keep `description` under 150 chars to avoid silent truncation when the context fills with many skills. Put the primary trigger phrase in the first 80 characters.

## Testing skills

1. Copy the skill to the appropriate path
2. Open OpenCode and type `/` to verify the skill appears
