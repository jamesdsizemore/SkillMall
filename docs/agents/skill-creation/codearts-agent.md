...
| | Value |
|--|-------|
| Project path | `.codeartsdoer/skills/` |
| Global path | `~/.codeartsdoer/skills/` |
| Native extensions | No — follows AgentSkills standard only |
| Install method | `cp -r skills/<category>/<slug> ~/.codeartsdoer/skills/` |

## Frontmatter support

CodeArts Agent follows the [AgentSkills open standard](https://agentskills.io/specification). Only standard fields are supported.
...
CodeArts Agent has no native extensions beyond the standard. Skills created using the [universal format](universal.md) work without modification.

For SkillMall skills targeting CodeArts Agent, use only:
...
CodeArts Agent has no extensions beyond the AgentSkills standard. There is no agent-specific skill format.

If the user asks for a "CodeArts Agent-specific" skill, create a universal skill and note in `compatibility` that it targets CodeArts Agent:

```yaml
compatibility: "Intended for use with CodeArts Agent."
```
...
Or describe the task — CodeArts Agent loads the skill automatically when the description matches.

## Description budget behavior

CodeArts Agent follows the universal best practice. All coding agents implement skill listing budgets. Keep `description` under 150 chars to avoid silent truncation when the context fills with many skills. Put the primary trigger phrase in the first 80 characters.

## Testing skills

1. Copy the skill to the appropriate path
2. Open CodeArts Agent and type `/` to verify the skill appears
