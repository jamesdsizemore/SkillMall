...
| Install method | `cp -r skills/<category>/<slug> ~/.junie/skills/` |

## Frontmatter support

Junie follows the [AgentSkills open standard](https://agentskills.io/specification). Only standard fields are supported.
...
Junie has no native extensions beyond the standard. Skills created using the [universal format](universal.md) work without modification.

For SkillMall skills targeting Junie, use only:
...
Junie has no extensions beyond the AgentSkills standard. There is no agent-specific skill format.

If the user asks for a "Junie-specific" skill, create a universal skill and note in `compatibility` that it targets Junie:

```yaml
compatibility: "Intended for use with Junie."
```
...
Or describe the task — Junie loads the skill automatically when the description matches.

## Description budget behavior

Junie follows the universal best practice. All coding agents implement skill listing budgets. Keep `description` under 150 chars to avoid silent truncation when the context fills with many skills. Put the primary trigger phrase in the first 80 characters.

## Testing skills

1. Copy the skill to the appropriate path
2. Open Junie and type `/` to verify the skill appears
