...
| Install method | `cp -r skills/<category>/<slug> ~/.mcpjam/skills/` |

## Frontmatter support

MCPJam follows the [AgentSkills open standard](https://agentskills.io/specification). Only standard fields are supported.
...
MCPJam has no native extensions beyond the standard. Skills created using the [universal format](universal.md) work without modification.

For SkillMall skills targeting MCPJam, use only:
...
MCPJam has no extensions beyond the AgentSkills standard. There is no agent-specific skill format.

If the user asks for a "MCPJam-specific" skill, create a universal skill and note in `compatibility` that it targets MCPJam:

```yaml
compatibility: "Intended for use with MCPJam."
```
...
Or describe the task — MCPJam loads the skill automatically when the description matches.

## Description budget behavior

MCPJam follows the universal best practice. All coding agents implement skill listing budgets. Keep `description` under 150 chars to avoid silent truncation when the context fills with many skills. Put the primary trigger phrase in the first 80 characters.

## Testing skills

1. Copy the skill to the appropriate path
2. Open MCPJam and type `/` to verify the skill appears
