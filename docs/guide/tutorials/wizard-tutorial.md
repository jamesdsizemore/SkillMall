...
Click `[ PREVIEW SKILL → ]`. The wizard advances to Step 4. No API call fires at this transition. The metadata you set (category and tags) is stored in session state.

---

## Step 4: SKILL.md Preview

Step 4 shows you a read-only preview of the SKILL.md file that will be generated. This is the skeleton of the skill — the frontmatter and the opening structure of the instructions body. The full content has not been generated yet (that happens in Step 5's API call), but you can see the description, name, category, and tags that will go into the file.
...
Read the description carefully. If it looks wrong — for example, if it starts with "This skill helps" rather than an imperative verb — note it, but do not worry yet. You can improve the description after the skill is created by editing SKILL.md directly. The quality score rubric awards 7 points for starting with an imperative verb; the research pipeline attempts to generate compliant descriptions, but occasionally produces a description that starts with a non-imperative phrase.
...
On the server, `/api/confirm-research` runs the Skill Builder and Prompt Engine in parallel. The Skill Builder generates SKILL.md, README.md, and the resource template and sample files. The Prompt Engine generates all prompt files — one per tool, one per category group, and one per selected meta type. Each prompt file requires an LLM call with the tool's extracted schema (name, inputs, outputs, artifact structure) embedded in the prompt. With seven tools and five meta types, that is roughly twelve LLM calls running in parallel where the provider's rate limits allow.
...
The file list is read-only in this view. You cannot expand files to preview their contents in Step 6 — this view is purely a manifest confirming which files will be written. If you want to review the SKILL.md content, use the Back button to return to Step 4 where the preview was shown.
...
When you click `[ CREATE SKILL ]`, the button text changes to `[ CREATING SKILL... ]` and both buttons become non-interactive. The browser fires a `POST` request to `/api/create-skill` with the same body that was sent to `/api/confirm-research`. The difference is what happens on the server: `/api/create-skill` actually writes files to disk using `atomicWrite` — the files are written to a temporary directory first, then moved atomically to `skills/business/jobs-to-be-done-framework-for-product-management/`. If any part of the write fails, no partial files are left behind.
